...
class _NewGoalFormState extends ConsumerState<_NewGoalForm> {
  ...
  Widget build(BuildContext context) {
    ...
    // Current weight is derived from the weight feed (it isn't a
    // field on `User`). Read it through the dedicated provider so a
    // weight log propagates here automatically. The estimator
  ...
  /// profile + the latest logged weight. Returns `null` when any
  /// required input is missing, matching [estimateCalories]'s
  /// contract.
